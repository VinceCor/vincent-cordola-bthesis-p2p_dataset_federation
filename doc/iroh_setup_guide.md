...
#### 6.53 The gossip task running in the background
...
**Wait before broadcasting:** If this node has bootstrap peers (`bootstrap.is_empty()`), it calls `receiver.joined().await` before sending anything. The first peer (not a bootstrap peer) skips this wait and broadcasts immediately, its manifest will be retrieved by subsquent peers as sonn as they connect.
...
**Receive:** `receiver.next().await` returns one event at a time. `Event::Received(message)` is the only case handled here. `serde_json::from_slice::<Manifest>(&message.content)` decodes them into a Manifest, and `save_peer_manifest` writes the JSON file to `data/peers_manifest`. Other event types (neighbor joining, leaving) are captured by `Ok(_) => {}` and ignored, as demonstrated in the official [`chat.rs` example](https://github.com/n0-computer/iroh-gossip/blob/main/examples/chat.rs).
...
#### 6.61 Expected directory stutcture per container
```bash
.
├── Cargo.toml
├── Dockerfile
...
#### 6.62 docker-compose.yml
Only the `environment` block for each service changes. peer1 starts without `BOOTSTRAP_PEERS`, the other point to it. This field should be modified when the first peer is launched.
...
```
In the logs `docker logs peer1`, locate the line with the `id: PublicKey(acf5d...)`:     
Copy the value of `PublicKey`

**Edit** `docker-compose.yml` and paste peer1 `PublicKey` into `BOOTSTRAP_PEERS` of the remaining services.
...
One the manifests have been propagated, a ticket contained in a file named `data/peers_manifest/<institution>.json` can be downloaded directly.
```bash
docker attach <peer>
fetch blobaazlu5wae...
