...
3. [client.py](#3-clientpy)
    - [3.1 P2PError](#31-p2perror)
    - [3.2 The constructor](#32-the-constructor)
    - [3.3 health(), files(), fetch()](#33-health-files-fetch)
    - [3.4 Private helpers](#34-private-helpers)
    - [3.5 raise for status](#35-raise-for-status)
    - [3.6 Result](#36-result)
        - [3.6.1 Install python library](#361-install-python-library)
        - [3.6.2 Manual test](#362-manual-test)
        - [3.6.3 Testing the error path](#363-testing-the-error-path)
4. [dataset.py](#4-datasetpy)
    - [4.1 Dependencies](#41-dependencies)
    - [4.2 Constants](#42-constants)
    - [4.3 files()](#43-files)
    - [4.4 get()](#44-get)
    - [4.5 Result](#45-result)
5. [Python query layer: load() and query()](#5-python-query-layer-load-and-query)
    - [5.1 load()](#51-load)
    - [5.2 query()](#52-query)
    - [5.3 federate()](#53-federate)
    - [5.4 filter_passenger() example of a custom function](#54-filter_passenger-example-of-a-custom-function)

## 1. Overview
> References: [Requests](https://requests.readthedocs.io/en/latest/)

The Rust node exposes three HTTP endpoints (`/health`, `/files`, `/fetch`), documented in the Axum HTTP server guide. `P2PClient` is the only piece of Python code that knows this node exists. It does not interpret the data it receives, it does not decide whether a file should be downloaded, it only translates Python method calls into HTTP requests and HTTP responses into Python values.

## 2. Dependencies
`requests` is the standard synchronous HTTP library for Python. Chosen over `aiohttp` or `httpx` because the project doesn't need async HTTP calls on the Python side.
```bash
...
class P2PError(Exception):
    ...
    pass
```
The caller (`P2PDataset`, or the researcher directly in a notebook) only needs to know "the node call failed", not whether it was a connection error, a timeout, or an HTTP 500. Catching one exception type is simpler than catching three.
...
`fetch()` is the only method that unwraps the response. The Rust node returns `{"path": "cache/<hash>.parquet"}`; `fetch()` returns `data["path"]` directly instead of the whole dict.
...
This is the part of the file that does the actual work. `get` and `post` are nearly identical, which is intentional.
```Python
...
`self.raise_for_status(response)`, called after the `try/except`, catches application-level failures: the node did respond, but with a non-200 status (e.g. the Axum `/fetch` handler returning `500` because the ticket was invalid).
...
```
It inspects a `Response` object and converts a non-200 status into a `P2PError` with a useful message. It tries to extract the "error" field the Rust API puts in its JSON error bodies, and falls back to the raw response text if the body isn't JSON, or doesn't contain that field.


### 3.6 Result
#### 3.6.1 Install python library
If you haven't already, install the requests library.
```bash
# Example of how to create a .venv
python3 -m venv .venv
...
### 4.2 Constants
> References: [python logging getLogger](https://docs.python.org/3/library/logging.html#logging.getLogger)
...
`get()` needs to stop as soon as a match is found (`break`). List comprehensions always walk the entire list. An explicit loop with `break` is more efficient and clearer when early exit is the goal.
```Python
...
```
`entry = f | {"institution": manifest.get("institution", "unknown")}`   
`f` is the file dict from the manifest. The `|` operator merges two dicts into a new one. The result is a flat dict with all four fields, so `get()` can write `entry["hash"]` and `entry["ticket"]` directly.
...
return cached
```
`entry['hash'][:16]` takes the first 16 characters of the BLAKE3 hash. The Rust node uses this exact slice to name the exported file. `cached.exists()` checks the disk: if the file is there, it is already a verified Parquet file and can be returned immediately with no network call.   
`self._client.fetch(entry["ticket"])` triggers `POST /fetch` on the Rust node. The return value (the relative path string) is not used here: `cached` was already computed from the hash and points to the same file. If the fetch fails, `P2PClient` raises `P2PError`, which propagates to the notebook: the user sees the full error rather than a silent `None`.
...
### 5.1 load()
`load()` is a thin wrapper around `get()`. If `get()` returns `None`, the file does not exist in any peer manifest. `load()` turns this into a `P2PError` rather than silently returning `None`, so the failure is visible immediately in the notebook. `pd.read_parquet(path)` reads the file directly from `cache/`; no network call happens here.
```Python
...
### 5.2 query()
`query()` takes one or more file names as arguments, for example `dataset.query("sample.parquet", "other_sample.parquet")`. The researcher is expected to call `dataset.files()` first to see what's available on the network, then pass the names they actually want.
```Python
# Fetch a specific set of files, chosen by name, from the network
# Each file is resolved and fetched independently (no merging)
...
### 5.3 federate()
`federate()` mirrors `query()`'s argument handling on purpose: at least one file name is required, each name is resolved through `get()`, and missing files are logged as a warning and skipped rather than failing the whole call. The difference is only in what happens after the files are on disk.  
...
# Shows how project-specific shortcuts can be added on top of the federated view without requiring the researcher
# to know SQL.
...
```
`con` is the connection returned by `federate()`, so the dataset view must already exist on that connection. `filter_passenger()` does nothing more than construct the same SQL query used manually in the demo notebook. It simply saves the researcher from having to write SQL for the specific filter.     
Use in notebook
```Python
con = dataset.federate("yellow_tripdata_2026-01.parquet", "yellow_tripdata_2026-02.parquet")
