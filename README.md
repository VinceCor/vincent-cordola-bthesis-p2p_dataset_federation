...
**Query Parquet files scattered across institutions from a single Juypter notebook - no central server, no data upload.**
...
This project explores a different model for sharing tabular data across institutions: instead of a central server that every site uploads to, each institutions keeps its Parquet files exactly where they already are, and a peer-to-peer network makes them discoverable and queryable as if they were local. A researcher opens a notebook, and the whole federation is queryable through it, no dataset has to move to a shared location, and no one has to run or maintain shared infrastructure.

The approach is not tied to any one dataset or filed. The motivating use case for this thesis is sharing [MRIQC](https://mriqc.readthedocs.io/en/latest/) Image Quality Metrics across neuroimaging labs, but any group of peers willing to share Parquet files over a decentralized network can reuse this node/client pair as-is.
...
- a **Rust node** - joins the iroh network, hashed and serves its local Parquet files, propagates a manifest of what it has via gossip, and exposes a small local HTTP API
...
Full test conditions and additional measurements in [`evaluation.md  1`](doc/evaluation.md#1-real-world-test-conducted)

## 7. Known limitations and future work

This is a proff of concept: the goal was a working end-to-end demonstration, not a production-ready system. Notably, manifest refresh is manual (no folder watcher yet), there's no application-level retry on failed downloads, and the local cache has no size limit. None of these are fundamental blockers - see [`evaluation.md 6-7`](doc/evaluation.md#6-known-limitations)

## 8. Academic context

This project is the Bachelor thesis of Vincent Cordola (ISC, HES-SO Valais-Wallis, 2025-26), carried out at McGill University / Origami lab, Montréal, under the supervision of Prof. Oscar Esteban, Prof. Jean-Baptiste Poline and Dr. Nikhil Bhagwat. Thanks to [Nathan Antonietti](https://github.com/NathanAnto) for taking part in the real-world Montréal, Switzerland test.

## 9. License

No license has been set yet - this section will be updated once one is chose.

> Claude chatbot was used to correct spelling errors and reformat the file, when the document had been completed a first time